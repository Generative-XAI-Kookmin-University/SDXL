...
if __name__ == "__main__":
    ...
    try:
        ...
        default_logger_cfgs = {
            ...
            # Add other loggers here if needed (e.g., tensorboard)
            # "tensorboard": {"target": "pytorch_lightning.loggers.TensorBoardLogger", "params": {"save_dir": logdir, "name": "tensorboard"}},
        }
        
        # Initialize logger if opt.wandb is True
        logger = None
        if opt.wandb:
            logger_cfg = default_logger_cfgs["wandb"]
            logger = instantiate_from_config(logger_cfg)
            # Config filtering for wandb
            wandb_config_dict = OmegaConf.to_container(config, resolve=True)
            keys_to_exclude = ['model', 'data', 'loss_fn_config', 'denoiser_config', 'network_config', 'sampler_config', 'conditioner_config']
            for key in keys_to_exclude:
                if key in wandb_config_dict:
                    del wandb_config_dict[key]
            
            # Pass the filtered config to wandb logger
            # Note: The init_wandb function seems to be a custom wrapper.
            # If the standard WandbLogger is used, config is passed here.
            # If init_wandb is needed, pass the filtered dict there.
            # Assuming standard WandbLogger usage with Trainer:
            # Trainer will handle passing config to the logger if provided in trainer_kwargs['logger']

        trainer_kwargs["logger"] = logger # Pass the initialized logger to trainer_kwargs
